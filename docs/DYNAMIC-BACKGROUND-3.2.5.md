...
1. Current media-derived Primary image endpoint, with a 550 ms fast-path load budget so a missing track-owned image cannot block inherited art for seconds.
2. Item-bound DOM artwork can commit immediately after identity validation.
3. Album-inherited Primary artwork shown in Jellyfin's live now-playing UI uses a dedicated short gate: at least 320 ms of current-media stability and 220 ms of stability for that exact artwork URL.
4. Generic unbound DOM artwork remains conservative: it must be new relative to the switch snapshot, the media must be stable for 900 ms, and that exact URL must remain stable for 1.5 s.
5. Confirmed no-art state is allowed only after repeated failures and a 2.5 s stability window.
...
The current now-playing artwork shown by Jellyfin is the first source of truth. This matters for music tracks that inherit Primary artwork from their album rather than owning a track-level image. A newly visible now-playing album image that was not present at the media-key switch is eligible immediately, and an already-loaded `<img>` element is reused directly instead of being fetched again.
